  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Learning Management</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage digital learning, live classes, and resources.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
        {[
          { title: 'Active Modules', value: '0', icon: BookOpen, color: 'text-primary-500', bg: 'bg-primary-50 dark:bg-primary-500/10' },
          { title: 'Live Sessions Today', value: '0', icon: Video, color: 'text-primary-500', bg: 'bg-primary-50 dark:bg-primary-500/10' },
          { title: 'Video Hours Viewed', value: '0', icon: Film, color: 'text-primary-500', bg: 'bg-primary-50 dark:bg-primary-500/10' },
          { title: 'Certificates Issued', value: '0', icon: Award, color: 'text-primary-500', bg: 'bg-primary-50 dark:bg-primary-500/10' },
        ].map((stat, idx) => (
          <motion.div 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Recent Videos</h2>
          </div>
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <Film className="w-12 h-12 text-slate-300 dark:text-slate-600 mb-3" />
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">No videos uploaded yet</p>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="px-6 py-4 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Schedule Live Class</h3>
                <button 
                  onClick={() => setIsModalOpen(false)}
              ...
              <form onSubmit={handleScheduleClass} className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Meeting Link (Auto-generated)</label>
                  <input type="text" value="https://meet.google.com/mock-link-123" readOnly className="w-full px-4 py-2 bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/10 rounded-xl outline-none text-slate-500 dark:text-slate-400 cursor-not-allowed" />
                </div>
                <div className="pt-4 flex justify-end gap-3">
                  ...
                  <button type="submit" className="px-5 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-primary-500/30 transition-all">Schedule Class</button>
                </div>
              </form>
            </motion.div>
          </div>
